...
- [ ] **跑马灯消息配置**:
  ...
  - [ ] **用户头像管理系统**:
    - [ ] **头像生成**: 使用DiceBear API生成
    - [ ] **虚拟头像库**: 后台上传管理虚拟用户头像（用于营造氛围）
    - [ ] **头像显示策略**: DiceBear生成头像 + 虚拟数据(上传头像) 混合展示
...
```
1. 用户点击邀请链接访问落地页
2. 系统识别邀请者信息并存储 ref_id
3. 显示个性化Toast："[朋友名]送你一个抽奖码"
4. 用户点击参与 → 生成 SESSION_ID（前端）
5. 立即唤醒 WhatsApp：wa.me/<business_number>?text=我要参与-[SESSION_ID]
6. 用户发送消息 → 后台接收并获取手机号
7. 完成激活绑定
8. WhatsApp 机器人回复激活成功
9. H5 页面通过 Realtime 监听到激活成功
10. 自动跳转到活动主页
```
...
- Hero Banner: 后台可配置的活动主视觉图片
  ...
  - **头像显示策略**: DiceBear生成头像 > 虚拟上传头像 > 默认头像
...
- **参与激活（P1）**: 直接唤醒 WhatsApp（wa.me/商家号?text=我要参与-[SESSION_ID]）
- **头像生成**: **DiceBear API集成**
...
### **技术约束**
- **移动端优先**: 必须完美适配移动端设备
- **激活路径（P1）**: WhatsApp优先（直接唤醒，不包含Google注册/登录）
- **头像获取策略**: DiceBear生成 > 虚拟头像库 > 默认头像
...
- **数据关联**: WhatsApp手机号与会话SESSION_ID必须完整关联
...
### **用户参与指标**
- **WhatsApp消息发送转化率** >85% (访问→成功唤醒并发送消息)
- **激活完成率** >80% (消息发送→后台激活完成)
- **头像生成成功率** >95% (DiceBear生成+虚拟兜底)
